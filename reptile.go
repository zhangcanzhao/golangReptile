...
const (
	baseUrl string = "https://weekend.ctrip.com/around/shanghai/huodong/p"
)
...
func main() {
	...
	for {
		...
		if response.StatusCode == 200 {
			...
			dom.Find(".basefix .searchresult_left").Each(func(i int, content *goquery.Selection) {
				title := content.Find(".searchlocation_fun a h2 strong").Text()
				// img := content.Find(".searchlocation_fun .product_pic img").Attr("src")
				fmt.Printf(title, "\n")
			})
		}
		page++
